...
git checkout -b fixed 4c9df9bfc63d2c3a9e20e20deaa854072e379f47 # Newer commits break some functionallity from eglot and from the custom banner
...
(setq doom-font (font-spec :family "JetBrainsMono Nerd Font" :size 17)
      ...
      ;; doom-serif-font (font-spec :family "JetBrainsMono Nerd Font" :size 22 :weight 'light)
)
#+end_src
...
Disable modeline on dashboard

#+begin_src emacs-lisp
(add-hook! 'doom-after-init-hook (hide-mode-line-mode 1))
#+end_src
...
In rainbow mode, colors cannot be displayed propperly over the highlight of hl-line-mode. Therfore we disable it.
#+begin_src emacs-lisp
(add-hook 'rainbow-mode-hook (lambda () (if (bound-and-true-p rainbow-mode)
                                            (hl-line-mode -1)
                                          (hl-line-mode t))))
#+end_src
...
#+begin_src emacs-lisp
(after! which-key
  (setq which-key-popup-type 'minibuffer)) ;; Remove pop-up glitch
#+end_src
...
(add-hook 'vertico-buffer-mode-hook 'hide-mode-line-mode)
#+end_src

**** Theming

Prefix the current candidate with ». From [[https://github.com/minad/vertico/wiki#prefix-current-candidate-with-arrow][github - vertico prefix]]
#+begin_src emacs-lisp
(vertico-buffer-mode)
...
;;  HACK working on NixOS if installing nixos.ccls
...
Emacs needs to fetch first the libraries installed from the system insead of the installed through stright.el used in the scheme module. Altering the =load-path= should fix the load order. For now I can't alter the load-path to load first system packages, so I'm pinning the packages to a specific commit.
#+begin_src emacs-lisp :tangle packages.el
...
(defun modify-files-with-dir (all-files dir lst)
  ;;lst should be '()
  ;;cons on the dir, but must later advance index by one
  (if (null (cdr all-files))
      (setq lst (cons  (cons dir (car all-files))   lst))
    (progn
      (setq lst (cons  (cons dir (car all-files))   lst))
      (modify-files-with-dir (cdr all-files) dir lst ))))

(defun collect-all-files (all-dirs all-files)
  ;;all-dirs: /var/log/guix/drvs/ab, ...ac, ...ad
  ;;all-files: '()
  (if (null (cdr all-dirs))
      (let* ((dir (car all-dirs))
         (str-len (length dir))
         (sub-dir (substring dir (- str-len 2) str-len))
         (dir-files (directory-files-and-attributes dir nil nil t))
         (dir-files-mod (modify-files-with-dir dir-files sub-dir '())))
    (setq all-files (append  dir-files-mod all-files)))
    (let* ((dir (car all-dirs))
       (str-len (length dir))
       (sub-dir (substring dir (- str-len 2) str-len))
       (dir-files (directory-files-and-attributes dir nil nil t))
       (dir-files-mod (modify-files-with-dir dir-files sub-dir '()))
       (all-files (append  dir-files-mod   all-files)))
      (collect-all-files (cdr all-dirs) all-files))))

(defun guix-last-log( )
  ;; Opens the most recent file under "prefix" which is /var/log/guix/drvs/ on Debian.
  ;; Paste into init.el and activate with M-x looklog.
  ;; https://stackoverflow.com/questions/30886282/emacs-lisp-how-can-i-get-the-newest-file-in-a-directory
  ;; https://emacs.stackexchange.com/questions/29555/how-to-run-sudo-commands-using-shell-command
  (interactive)
  (let* ((prefix "/var/log/guix/drvs")
         (all-dirs (directory-files prefix t "[A-Za-z0-9]" nil))
         (lst '())
         (all-files (collect-all-files all-dirs lst))
         (newest-file-as-list   (seq-find
                                 '(lambda (x) (not (nth 2 x))) ; non-directory
                                 (sort all-files '(lambda (x y) (time-less-p (nth 6 y) (nth 6 x))))))
         (newest-file-subdir (car newest-file-as-list))
         (newest-file  (concat prefix "/" newest-file-subdir "/" (cadr newest-file-as-list)))
         (newest-file-length (length newest-file))
         (isbz2? (string= "bz2" (substring newest-file (- newest-file-length 3) newest-file-length )))
         (default-directory "/sudo::")
         (dummy (if isbz2? (shell-command (concat "bzip2 -d " newest-file))))
         (file-to-display (if isbz2? (substring newest-file 0 -4) newest-file))
         (buffer-name "logbuffer"))
    (progn
     (message file-to-display)
     (switch-to-buffer (get-buffer-create buffer-name))
     (erase-buffer)
     (insert-file-contents file-to-display)
     (goto-char (point-max))
     (with-current-buffer buffer-name (guix-build-log-mode)))))
#+end_src
